<template>
    <div>
      <div class="flex h-fit justify-between flex-wrap items-center py-2">
        ...
        <div class="modal-groups">
          <TagModal :load-data="FetchData" :formMode="formMode" :user="item" ref="formModal1"></TagModal>
          <Button @click="showCreateModal()" type="submit" text="Create Tag"  btnClass="btn-primary h-10 flex items-center"/>
        </div>
      </div>
      <Card noborder>
        <div class="md:flex justify-between pb-6 md:space-y-0 space-y-3 items-center">
          <h5>Thread Categories</h5>
  ...
  import TagModal from "./TagsFormModal.vue";
  ...
  export default {
    ...
    methods: {
        async FetchData(){
            await apiClient.get(`/api/admin/forum/tags`, { withCredentials: true })
            .then((response) => {
        ...
        async deleteItem(itemId) {
          await apiClient.delete(`/api/admin/forum/tag/delete/${itemId}`, { withCredentials: true })
          .then((response) => {
              //this.users = response.data.users;
              //this.users = this.users.filter(item => item.id !== itemId);
              this.FetchData();
              console.table(this.categories);
              this.toast.success('Tag has been deleted successfully', { timeout: 2000 });
          }).catch((error) => {
              this.toast.error('Failed to delete Tag', { timeout: 2000 });
              console.log(error);
          });
        },
    },
