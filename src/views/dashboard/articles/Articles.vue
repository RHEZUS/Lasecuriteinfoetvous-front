...
import ArticleModal from "./ArticleFormModal.vue";
...
export default {
  ...
  methods: {
      async FetchData(){
          await apiClient.get(`/api/admin/articles`, { withCredentials: true })
          .then((response) => {
      ...
      async deleteItem(itemId) {
        await apiClient.delete(`/api/admin/article/delete/${itemId}`)
        .then(response => {
            this.FetchData();
            console.log(response);
            this.toast.success('Article has been deleted successfully', { timeout: 2000 });
        }).catch(error => {
            this.toast.error('Failed to delete Article', { timeout: 2000 });
            console.log(error);
        });
      },
  },
